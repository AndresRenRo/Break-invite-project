    ...
    <div onClick={phase === "sealed" ? handleTap : undefined} style={{
      ...
      {phase === "sealed" && (
        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:"1.5rem",animation:"fadeUp .8s ease forwards",zIndex:10 }}>
          <div style={{fontSize:".42rem",letterSpacing:".4em",textTransform:"uppercase",color:"#F2C744",opacity:.4}}>You've received a Break Invite</div>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:".3rem"}}>
            <p style={{fontSize:".65rem",color:"#E8DCC8",opacity:.5}}>From <span style={{color:b.color,fontWeight:700}}>{senderName}</span></p>
            <p style={{fontSize:".55rem",color:"#E8DCC8",opacity:.25,animation:"tapPulse 2s ease-in-out infinite"}}>Tap to open</p>
          </div>
        </div>
      )}
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"1rem",maxWidth:"380px",width:"100%",animation:"cardUp .6s cubic-bezier(.16,1,.3,1) forwards",zIndex:10}}>
          <div style={{background:`linear-gradient(180deg,${b.bg},#0a0a0a)`,border:`1px solid ${b.color}33`,borderRadius:"8px",padding:"2rem 1.5rem",width:"100%",textAlign:"center",position:"relative",boxShadow:`0 12px 40px rgba(0,0,0,.5)`}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:".5rem",marginBottom:"1rem",opacity:.15}}>
              <div style={{width:"25px",height:"1px",background:b.color}}/><span style={{fontSize:".35rem",letterSpacing:".3em",textTransform:"uppercase",color:b.color}}>Break Invite</span><div style={{width:"25px",height:"1px",background:b.color}}/>
            </div>
            <div style={{fontSize:"3.2rem",marginBottom:".6rem",animation:"bounceIn .5s ease .2s both"}}>{b.emoji}</div>
            <p style={{fontSize:".65rem",color:"#E8DCC8",opacity:.45,marginBottom:".3rem",animation:"fadeUp .5s ease .4s both"}}>
              <span style={{fontWeight:700,color:b.color}}>{senderName}</span> has invited you to a
            </p>
            <h1 style={{fontFamily:"'Playfair Display','Georgia',serif",fontSize:"clamp(1.3rem,4.5vw,1.8rem)",color:"#E8DCC8",marginBottom:".3rem",textShadow:`0 0 20px ${b.color}33`,animation:"fadeUp .5s ease .5s both"}}>{b.label}</h1>
            <p style={{fontSize:".5rem",color:b.color,opacity:.5,marginBottom:".8rem",animation:"fadeUp .5s ease .55s both"}}>{mins} minutes</p>
            <p style={{fontFamily:"'Georgia',serif",fontStyle:"italic",fontSize:".68rem",lineHeight:1.6,color:"#E8DCC8",opacity:.45,marginBottom:"1.3rem",animation:"fadeUp .5s ease .6s both"}}>"{b.invite}"</p>
            <button onClick={onOpen} style={{fontFamily:"'Space Mono',monospace",fontSize:".55rem",fontWeight:700,letterSpacing:".18em",textTransform:"uppercase",background:b.color,color:"#fff",border:"none",padding:".85rem 2.2rem",cursor:"pointer",boxShadow:`0 4px 20px ${b.color}44`,animation:"fadeUp .5s ease .7s both"}}>Accept the invitation</button>
          </div>
        </div>
      )}
    </div>
